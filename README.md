...
**Status:** v0.1.2 — working and tested. 8 AWS resource kinds, 5 failure scenarios, a GitHub Action
that runs every scenario on a pull request and posts the verdict, and a web viewer. **83 tests
(63 Rust, 20 Python), plus 6 for the viewer; CI green.**
...
jobs:
  helios:
    ...
    steps:
      ...
      - uses: veerarakesh56/helios/action@v0.1.2
        with:
...
**83 tests, all green in CI (plus 6 for the web viewer):**

| | |
|---|---|
| Rust | **63** across the graph, engine, SMT encoding, verify loop and CLI |
